...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="patch size used in training deepif. "
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="model_zoo/deepif/001_classicalSR_DIV2K_s48w8_deepif-M_x2.pth",
    )
    parser.add_argument(
        "--folder_lq",
        type=str,
    ...
    for idx, path in enumerate(sorted(glob.glob(os.path.join(folder, "*")))):
        ...
        # pad input image to be a multiple of 64, and H=W
        h_old, w_old, _ = img_lq.shape
        if h_old > w_old:
            h_pad = math.ceil(h_old / 64) * 64 - h_old
            w_pad = math.ceil(h_old / 64) * 64 - w_old
        else:
            h_pad = math.ceil(w_old / 64) * 64 - h_old
            w_pad = math.ceil(w_old / 64) * 64 - w_old
        h_new = h_old + h_pad
        img_lq = torch.from_numpy(img_lq).float()
        img_lq = torch.cat([img_lq, torch.flip(img_lq, [0])], 0)[: h_old + h_pad, :, :]
        img_lq = torch.cat([img_lq, torch.flip(img_lq, [1])], 1)[:, : w_old + w_pad, :]

        img_lq = Image.fromarray((img_lq * 255.0).numpy().round().astype(np.uint8))
        # inference
        middle_res = super_resolution(
            ...
            support_pil_img=img_lq,
            img_scale=args.scale,
            img_size=h_new,
            if_III_kwargs={
                "sample_timestep_respacing": "smart100",
                "aug_level": 0, # 0.5
                "guidance_scale": 1.0, # 6.0
            },
            # if_III_kwargs={
            #     "guidance_scale": 4.0,
        ...
        output = np.array(middle_res["III"][0])[
            : h_old * args.scale, : w_old * args.scale, :
        ]
        # save image
        output = output.round().astype(np.uint8)
        output = output[:, :, ::-1]
        cv2.imwrite(f"{save_dir}/{imgname}_DeepIF.png", output)
...
def setup(args):
    ...
    if args.task in ["classical_sr", "lightweight_sr"]:
        save_dir = f"results/deepif_{args.task}_x{args.scale}"
    ...
    # 004 grayscale image denoising/ 005 color image denoising
        ...
        save_dir = f"results/deepif_{args.task}_noise{args.noise}"
        folder = args.folder_gt
        border = 0
        window_size = 8
    ...
    # 006 JPEG compression artifact reduction
        ...
        save_dir = f"results/deepif_{args.task}_jpeg{args.jpeg}"
        folder = args.folder_gt
        border = 0
        window_size = 7
